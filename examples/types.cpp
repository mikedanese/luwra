...
namespace luwra {
// You may add custom specializations of luwra::Value in order to retrieve them
// from the stack.
template <> struct Value<char> {
  /**
   * Retrieve the value at position `n`.
   */
  static inline char read(lua_State *state, int n) {
   ...
   */
  static inline size_t push(lua_State *state, char val) {
    if (val == 0)
      return 0;
...
static void read_chars(char a, char b) {
