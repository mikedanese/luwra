...
struct GlobalAccessor {
  ...
  inline GlobalAccessor(State *state, const std::string &key)
      : state(state), key(key) {}
   ...
   * Shortcut for `set()`
   */
  template <typename V> inline GlobalAccessor &operator=(V value) {
    return set<V>(value);
  }
   ...
   * Shortcut for `get()`
   */
  template <typename V> inline operator V() { return get<V>(); }
};
...
struct StateWrapper {
   ...
   */
  inline StateWrapper(State *state) : state(state), close_state(false) {}

  /**
   * Create a new Lua state.
   */
  inline StateWrapper() : state(luaL_newstate()), close_state(true) {}
  ...
  inline operator State *() { return state; }

  inline void loadStandardLibrary() { luaL_openlibs(state); }
   ...
   */
  inline GlobalAccessor operator[](const std::string &key) const {
   ...
   */
  template <typename T>
  inline void registerUserType(const std::string &ctor_name,
   ...
   */
  inline int runString(const std::string &code) {
   ...
   * Execute a file.
   */
  inline int runFile(const std::string &filepath) {
