...
namespace internal {
template <typename T> using StripUserType = typename std::remove_cv<T>::type;

/**
 * User type registry identifiers
 */
template <typename T> struct UserTypeReg {
  /**
   * Dummy field which is used because it has a seperate address for each
   * instance of T
   */
  static const int id;
...
template <typename T> const int UserTypeReg<T>::id = INT_MAX;
 ...
 */
template <typename U> static inline void new_user_type_metatable(State *state) {
 ...
 */
template <typename U>
static inline StripUserType<U> *check_user_type(State *state, int index) {
  using T = StripUserType<U>;
 ...
 */
template <typename U>
static inline void apply_user_type_meta_table(State *state) {
...
static inline int construct_user_type(State *state) {
  return direct<size_t(A...)>(
      state, &Value<StripUserType<U> &>::template push<A...>, state);
}

/**
 * Lua C function to destruct a user type T
 */
template <typename U> static inline int destruct_user_type(State *state) {
  using T = StripUserType<U>;
 ...
 */
template <typename U> static int stringify_user_type(State *state) {
  using T = StripUserType<U>;

  return push(state, internal::UserTypeReg<T>::name + "@" +
 ...
 * User type
 */
template <typename U> struct Value<U &> {
   ...
   */
  static inline U &read(State *state, int n) {
   ...
   * \note Instances created using this specialization are allocated and
   * constructed as full user
   *       data types in Lua. The default garbage-collecting hook will destruct
   * the user type,
   ...
   */
  template <typename... A>
  static inline size_t push(State *state, A &&... args) {
 ...
 */
template <typename U, typename... A>
static inline internal::StripUserType<U> &construct(State *state,
                                                    A &&... args) {
  using T = internal::StripUserType<U>;
...
template <typename U> struct Value<U *> {
  ...
  /**
   * Copy a value onto the stack. This function behaves exactly as if you would
   * call
   ...
   */
  static inline size_t push(State *state, const T *ptr) {
...
/**
 * Register the metatable for user type `T`. This function allows you to
 * register methods
 * which are shared across all instances of this type.
 *
 * By default a garbage-collector hook and string representation function are
 * added as meta methods.
...
template <typename U>
static inline void
registerUserType(State *state, const MemberMap &methods = MemberMap(),
...
namespace internal {
template <typename T> struct UserTypeSignature {
  static_assert(sizeof(T) == -1,
                "Parameter to UserTypeSignature is not a valid signature");
};

template <typename T, typename... A> struct UserTypeSignature<T(A...)> {
...
/**
 * Same as the other `registerUserType` but registers the construtor as well.
 * The template parameter
 * is a signature `U(A...)` where `U` is the user type and `A...` its
 * constructor parameters.
 */
template <typename T>
static inline void
registerUserType(State *state, const std::string &ctor_name,
...
#define LUWRA_WRAP_CONSTRUCTOR(type, ...)                                      \
  (&luwra::internal::construct_user_type<luwra::internal::StripUserType<type>, \
                                         __VA_ARGS__>)

#define LUWRA_FIELD(type, name)                                                \
  { __STRING(name), LUWRA_WRAP_FIELD(type::name) }
#define LUWRA_METHOD(type, name)                                               \
  { __STRING(name), LUWRA_WRAP_METHOD(type::name) }
#define LUWRA_FUNCTION(type, name)                                             \
  { __STRING(name), LUWRA_WRAP_FUNCTION(type::name) }
