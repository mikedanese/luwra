...
namespace internal {
template <typename T> struct GenericWrapper {
...
struct GenericWrapper<R(A...)> : FunctionWrapper<R(A...)> {};
...
template <typename T, typename R, typename... A>
struct GenericWrapper<R (T::*)(A...)> : MethodWrapper<R (T::*)(A...)> {};
...
#define LUWRA_WRAP(entity)                                                     \
  (&luwra::internal::GenericWrapper<decltype(                                  \
       &entity)>::template invoke<&entity>)

/**
 * Generate a user type member manifest (pair).
 * \param type Qualified struct/class name
 * \param name Member name
 * \returns `std::pair<const char*, CFunction>` which can be used to register a
 * user type member
 */
