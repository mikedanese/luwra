...
template <typename R, typename... A> struct FunctionWrapper<R(A...)> {
  template <R (*fun)(A...)> static inline int invoke(State *state) {
...
template <typename S> struct NativeFunction {
 ...
 */
template <typename R, typename... A> struct NativeFunction<R(A...)> {
  State *state;
  int index;
 ...
 * \note This value is only available as long as it exists on the stack.
 */
template <typename... A> struct NativeFunction<void(A...)> {
  State *state;
  int index;
...
template <typename R, typename... A> struct Value<NativeFunction<R(A...)>> {
  static inline NativeFunction<R(A...)> read(State *state, int index) {
