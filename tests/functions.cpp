...
static int noret_environment = 0;

static void test_function_noret_noparams() { noret_environment++; }

static void test_function_noret(int a, int b) { noret_environment = a + b; }

static int test_function_noparams() { return 13 * 37; }

static int test_function(int a, int b) { return (a + b) * (a - b); }
...
TEST_CASE("NativeFunction") {
  ...
  SECTION("with return value") {
    REQUIRE(luaL_dostring(state, "return function (x, y) return x + y end") ==
            LUA_OK);
  ...
  SECTION("without return value") {
    REQUIRE(luaL_dostring(state,
                          "return function (x, y) returnValue = x + y end") ==
            LUA_OK);
