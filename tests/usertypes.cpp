...
struct B {
  ...
  B(int val) : n(val), cn(val), vn(val), cvn(val) {}
};
...
TEST_CASE("UserTypeFields") {
  ...
  // Registration
  luwra::registerUserType<B>(state,
                             {LUWRA_MEMBER(B, n), LUWRA_MEMBER(B, cn),
                              LUWRA_MEMBER(B, vn), LUWRA_MEMBER(B, cvn)});
...
struct C {
  ...
  int foo1(int x) { return prop += x; }

  int foo2(int x) const { return prop + x; }

  int foo3(int x) volatile { return prop -= x; }

  int foo4(int x) const volatile { return prop - x; }
};
...
TEST_CASE("UserTypeMethods") {
  ...
  // Registration
  luwra::registerUserType<C>(state,
                             {LUWRA_MEMBER(C, foo1), LUWRA_MEMBER(C, foo2),
                              LUWRA_MEMBER(C, foo3), LUWRA_MEMBER(C, foo4)});
