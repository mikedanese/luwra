...
// Nil
template <> struct Value<std::nullptr_t> {
  static inline std::nullptr_t read(State *state, int n) {
    luaL_checktype(state, n, LUA_TNIL);
    return nullptr;
  }

  static inline size_t push(State *state, std::nullptr_t) {
    lua_pushnil(state);
    return 1;
  }
};
 ...
 */
template <typename T> static inline size_t push(State *state, T value) {
  return Value<T>::push(state, value);
}
 ...
 */
template <typename T> static inline T read(State *state, int index) {
  return Value<T>::read(state, index);
}

/**
 * Define a template specialization of `Value` for `type` with a `retrf(State*,
 * int)` which
 * extracts it from the stack and a `pushf(State*, type)` which pushes the value
 * on the stack again.
...
#define LUWRA_DEF_VALUE(type, retrf, pushf)                                    \
  template <> struct Value<type> {                                             \
    static inline type read(State *state, int n) { return retrf(state, n); }   \
                                                                               \
    static inline size_t push(State *state, type value) {                      \
      pushf(state, value);                                                     \
      return 1;                                                                \
    }                                                                          \
  }
...
namespace internal {
template <typename T> struct NumericTransportValue {
  static_assert(
      sizeof(T) == -1,
...
template <> struct NumericTransportValue<Integer> {
  static inline Integer read(State *state, int index) {
    return luaL_checkinteger(state, index);
  }

  static inline size_t push(State *state, Integer value) {
...
template <> struct NumericTransportValue<Number> {
  static inline Number read(State *state, int index) {
    return luaL_checknumber(state, index);
  }

  static inline size_t push(State *state, Number value) {
    lua_pushnumber(state, value);
    return 1;
  }
};

// Base for `Value<I>` specializations which uses `B` as transport unit, where
// `I` is smaller
// than `B`.
template <typename I, typename B> struct NumericContainedValueBase {
  static constexpr bool qualifies =
      // TODO: Remove warning about comparsion between signed and unsigned
      // integers
      std::numeric_limits<I>::max() <= std::numeric_limits<B>::max() &&
      std::numeric_limits<I>::lowest() >= std::numeric_limits<B>::lowest();

  static inline I read(State *state, int index) {
    return static_cast<I>(NumericTransportValue<B>::read(state, index));
  }

  static inline size_t push(State *state, I value) {
...
// `I` is bigger
// than `B`.
template <typename I, typename B> struct NumericTruncatingValueBase {
  static inline I read(State *state, int index) {
    return static_cast<I>(NumericTransportValue<B>::read(state, index));
  }

  static inline size_t push(State *, I) {
    static_assert(sizeof(I) == -1, "You must not use 'Value<I>::push' "
                                   "specializations which inherit from "
...
using NumericValueBase = typename std::conditional<
    std::is_same<I, B>::value, NumericTransportValue<B>,
    typename std::conditional<NumericContainedValueBase<I, B>::qualifies,
                              NumericContainedValueBase<I, B>,
                              NumericTruncatingValueBase<I, B>>::type>::type;
}
...
#define LUWRA_DEF_NUMERIC(base, type)                                          \
  template <> struct Value<type> : internal::NumericValueBase<type, base> {};
...
template <size_t n> struct Value<char[n]> : Value<const char *> {};

// Alias for const string literals
template <size_t n> struct Value<const char[n]> : Value<const char *> {};

/**
 * C Functions may be pushed aswell.
 */
template <> struct Value<CFunction> {
  static inline size_t push(State *state, CFunction fun) {
 ...
 * Note: this value is only available as long as it exists on its originating
 * stack.
 */
struct Arbitrary {
  /**
 ...
 */
template <> struct Value<Arbitrary> {
  static inline Arbitrary read(State *state, int index) {
    if (index < 0)
  ...
  static inline size_t push(State *state, const Arbitrary &value) {
...
namespace internal {
template <typename> struct StackPusher;

template <size_t I> struct StackPusher<IndexSequence<I>> {
  template <typename... T>
  static inline size_t push(State *state, const std::tuple<T...> &package) {
    using R = typename std::tuple_element<I, std::tuple<T...>>::type;
    return Value<R>::push(state, std::get<I>(package));
  }
};

template <size_t I, size_t... Is> struct StackPusher<IndexSequence<I, Is...>> {
  template <typename... T>
  static inline size_t push(State *state, const std::tuple<T...> &package) {
    return StackPusher<IndexSequence<I>>::push(state, package) +
 ...
 */
template <typename... A> struct Value<std::tuple<A...>> {
  static inline size_t push(State *state, const std::tuple<A...> &value) {
 ...
 */
template <typename T> struct Value<const T> : Value<T> {};

/**
 * Fix specialization for volatile types.
 */
template <typename T> struct Value<volatile T> : Value<T> {};

namespace internal {
struct PushableI {
  virtual size_t push(State *state) const = 0;

  virtual PushableI *copy() const = 0;

  virtual ~PushableI() {}
};

template <typename T> struct PushableT : virtual PushableI {
  T value;

  inline PushableT(T value) : value(value) {}

  virtual size_t push(State *state) const {
    return Value<T>::push(state, value);
  }

  virtual PushableI *copy() const { return new PushableT<T>(value); }
};
}
...
struct Pushable : virtual internal::PushableI {
  ...
  template <typename T>
  inline Pushable(T value)
      : interface(new internal::PushableT<T>(value)) {}

  inline Pushable(Pushable &&other) : interface(other.interface) {
    other.interface = nullptr;
  }

  Pushable(const Pushable &other) : interface(other.interface->copy()) {}

  virtual size_t push(State *state) const { return interface->push(state); }

  virtual internal::PushableI *copy() const { return new Pushable(*this); }
...
template <> struct Value<Pushable> {
  static inline size_t push(State *state, const Pushable &value) {
